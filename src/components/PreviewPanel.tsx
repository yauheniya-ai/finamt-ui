...
// ---------------------------------------------------------------------------
// PDF fullscreen overlay with data panel
// ---------------------------------------------------------------------------
function PDFFullscreen({ url, receipt, onClose, onEdit }: {
  url: string; receipt: Receipt; onClose: () => void; onEdit?: () => void;
}) {
  const splits = ((receipt as Receipt & { vat_splits?: { position: number; vat_rate: number | null; vat_amount: number | null }[] }).vat_splits ?? []);
  return (
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center justify-between px-4 py-2 bg-black border-b border-white/10 shrink-0">
          <span className="text-white text-xs font-mono font-bold truncate">
            {receipt.vendor ?? receipt.counterparty?.name ?? receipt.id.slice(0, 16)}
          </span>
          <Tip label="Exit fullscreen" pos="bottom">
            <button onClick={onClose} className="text-white/60 hover:text-white transition-colors ml-4 shrink-0">
              <Icon icon="mdi:fullscreen-exit" className="w-5 h-5" />
            </button>
          </Tip>
        </div>
        <iframe src={url} className="flex-1" title="Receipt PDF fullscreen" />
      </div>

      {/* Data panel */}
      <div className="w-[320px] shrink-0 bg-white border-l-2 border-black flex flex-col overflow-hidden">
        <div className="px-3 py-2 bg-amber-400 border-b border-black/10 shrink-0 flex items-center justify-between">
          <p className="text-xs font-black text-black uppercase tracking-wider">Extracted Data</p>
          <button onClick={() => { onClose(); onEdit?.(); }}
            className="bg-red-500 text-white p-1 rounded hover:bg-red-600 transition-colors"
            title="Edit">
            <Icon icon="fe:edit" className="w-3.5 h-3.5" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto p-3 flex flex-col gap-3 min-h-0">
          {/* Counterparty */}
          <div>
            <p className="text-[10px] font-black uppercase tracking-wider text-black/40 mb-1">Counterparty</p>
            <p className="text-xs font-bold">{receipt.vendor ?? receipt.counterparty?.name ?? "—"}</p>
            {receipt.counterparty?.vat_id && <p className="text-xs font-mono text-black/50">{receipt.counterparty.vat_id}</p>}
            {receipt.counterparty?.tax_number && <p className="text-xs font-mono text-black/50">{receipt.counterparty.tax_number}</p>}
          </div>
          {/* Receipt */}
          <div>
            <p className="text-[10px] font-black uppercase tracking-wider text-black/40 mb-1">Receipt</p>
            {receipt.receipt_date && <div className="flex justify-between text-xs"><span className="text-black/50">Date</span><span className="font-mono">{receipt.receipt_date}</span></div>}
            {receipt.receipt_number && <div className="flex justify-between text-xs"><span className="text-black/50">No.</span><span className="font-mono">{receipt.receipt_number}</span></div>}
          </div>
          {/* Amounts */}
          <div>
            <p className="text-[10px] font-black uppercase tracking-wider text-black/40 mb-1">Amounts</p>
            <div className="flex justify-between text-xs"><span className="text-black/50">Total</span><span className="font-mono font-bold">{fmt(receipt.total_amount)}</span></div>
            {splits.length > 0 ? splits.map((s, i) => (
              <div key={i} className="flex justify-between text-xs">
                <span className="text-black/50">VAT {s.position}</span>
                <span className="font-mono">{s.vat_rate != null ? `${s.vat_rate}%` : ""}{s.vat_amount != null ? ` · ${fmt(s.vat_amount)}` : ""}</span>
              </div>
            )) : (
              <div className="flex justify-between text-xs">
                <span className="text-black/50">VAT</span>
                <span className="font-mono">{receipt.vat_percentage != null ? `${receipt.vat_percentage}%` : "—"} · {fmt(receipt.vat_amount)}</span>
              </div>
            )}
            <div className="flex justify-between text-xs"><span className="text-black/50">Net</span><span className="font-mono">{fmt(receipt.net_amount)}</span></div>
          </div>
          {/* Items */}
          {receipt.items?.length > 0 && (
            <div>
              <p className="text-[10px] font-black uppercase tracking-wider text-black/40 mb-1">Items ({receipt.items.length})</p>
              {receipt.items.map((item, i) => (
                <div key={i} className="flex justify-between text-xs py-0.5 border-b border-black/5">
                  <span className="text-black/30 font-mono w-4 shrink-0">{item.position ?? i+1}.</span>
                  <span className="text-black/70 flex-1 truncate mx-1">{item.description || "—"}</span>
                  <span className="font-mono shrink-0">{fmt(item.total_price)}</span>
                </div>
              ))}
            </div>
          )}
...
export default function PreviewPanel({ receipt, apiBase, dbPath, onSaved }: Props) {
  ...
  // Sync localVerified when receipt prop changes (e.g. after onSaved refresh)
  ...
  const startEditing = () => {
    ...
    setSaved(false); setSaveErr(null); setEditing(true);
  };
  ...
  const applyVerifiedCp = (cp: VerifiedCp) => {
    ...
    // Selecting from verified means this counterparty is verified — reflect immediately
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaved(true); setSavedVisible(true); setEditing(false); onSaved?.(updated);
  ...
  return (
    <>
      {pdfOpen && pdfUrl && (
        <PDFFullscreen url={pdfUrl} receipt={receipt} onClose={() => setPdfOpen(false)} onEdit={() => { setPdfOpen(false); if (!editing) startEditing(); }} />
      )}

      <aside className="w-[380px] shrink-0 bg-white border-l-2 border-red-500 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-4 min-h-0">
          ...
          <section>
            <div className="flex items-center justify-between mb-2">
              ...
              {receipt.counterparty?.id && (
                <div className="flex items-center gap-1.5">
                  {verifyConfirm ? (
                    <span className="flex items-center gap-1.5 text-[10px] font-bold text-black">
                      Confirm verified?
                      <button onClick={async () => {
                          setLocalVerified(true);   // optimistic — update immediately
                          setVerifyConfirm(false);
                          await fetch(`${apiBase}/counterparties/${receipt.counterparty!.id}/verify${qs(dbPath)}`, {
                            method: "PATCH", headers: { "Content-Type": "application/json" },
                            body: JSON.stringify({ verified: true }),
                          }).catch(() => { setLocalVerified(null); }); // revert on error
                          onSaved?.(receipt);
                        }}
                    ...
                    <label className="flex items-center gap-1.5 cursor-pointer select-none text-[10px] font-bold text-black/60 uppercase tracking-wider">
                      ...
                      <input type="checkbox" checked={isVerified} className="sr-only"
                        onChange={() => {
                          if (!isVerified) {
                            ...
                            fetch(`${apiBase}/counterparties/${receipt.counterparty!.id}/verify${qs(dbPath)}`, {
                              method: "PATCH", headers: { "Content-Type": "application/json" },
